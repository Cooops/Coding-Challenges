...
def alphabet_war(fight):
    ...
    leftScore = 0
    for score in fight:
        if score in right.keys():
            rightScore += right[score]
        elif score in left.keys():
            leftScore += left[score]
    return "Let's fight again!" if (leftScore-rightScore == 0) else "Left side wins!" if (leftScore>rightScore) else "Right side wins!"
